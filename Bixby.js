//const {AdMobBanner} = require('AdMobBanner')
        ...
        */
            for (let anji of setik){
                if (budy === anji)
                {
                    switch(anji)
                    {
                        case 'Gm': case 'gm': case 'Good morning': case 'ගුඩ් මෝනින්':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/gm.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                                GojoMdNx.sendMessage(m.chat, { text: `*🎤දහසක් මල්🌸 විකසිත වන😊 මේ හිමිදිරි උදෑසනේ🌄 පතන්නම් ඔබ සැමට🙏 සුභම සුභ උදෑසනක් කියා...😃*` }, { quoted: m })
                            }break
                        case 'anony': case 'Shyaman': case 'shyaman': case 'Shyamn':
                            {
                                result = fs.readFileSync(`./BixbyMedia/image/Anony.jpg`)
                                GojoMdNx.sendMessage(m.chat, { image: result }, { quoted: m })
                            }break
                        case 'hi': case 'Hi': case 'හායි': case 'Hii':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/Hi.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                        ...
                        case 'ane': case 'Ane': case 'අනේ':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/ane.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                                GojoMdNx.sendMessage(m.chat, { text: '*මොකෝ අනේ* 😏 '},{quoted:m})
                            }break
                        case 'Menika': case 'මැණික': case 'Manika': case 'Sudu': case 'සුදූ':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/sudu.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                                GojoMdNx.sendMessage(m.chat, { text: '*ඔව් පැටියෝ* 🥺 '},{quoted:m})
                            }break
                        case 'Gn': case 'gn': case 'Good night': case 'ගුඩ් නයිට්':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/gn.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                                GojoMdNx.sendMessage(m.chat, { text: '*සුබ රාත්‍රියක්* 😘💫 '},{quoted:m})
                            }break
                        case 'පලයන්': case 'පල': case 'Pala': case 'Palayn': case 'pala': case 'Pamka': case 'Pakaya': case "Hutta": case 'පම්ක': case 'පකයා':
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/pala.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                            }break
                        case 'Ko': case 'ko': case 'කෝ': case 'Koo': 
                            {
                                result = fs.readFileSync(`./BixbyMedia/sticker/ko.webp`)
                                GojoMdNx.sendMessage(m.chat, { sticker: result }, { quoted: m })
                                GojoMdNx.sendMessage(m.chat, { text: '*කෝ නෑනේ* 🌝🤌 '},{quoted:m})
                            }break
                        case 'ba': case 'Ba': case 'බැ': case 'බෑ': case 'බැහැ': 
                            {
                            GojoMdNx.sendMessage(m.chat, { text: `*බැරි නම් නිකං ඉන්න😏*` }, { quoted: m })
                            }break
                        case 'ado': case 'Ado': case 'අඩෝ':
                             {
                            GojoMdNx.sendMessage(m.chat, { text: `*මොකෝ අඩෝ😏*` }, { quoted: m })
                            }break
                        case 'danna': case 'danne na': case 'දන්නෑ': case 'දන්නෙ නෑ':
                             {
                            GojoMdNx.sendMessage(m.chat, {text: `*මං දන්නෙත් නෑ☹️*` }, { quoted: m })
                            }break
                        case '🥰': case '😗': case '😙': case '😘😘': case '😘': case '😍': case 'umma': case '❤️':
                             {
                            GojoMdNx.sendMessage(m.chat, {text: `*I Love You Umma 😘😘*` }, { quoted: m })
                            }break
                }
			}}
			for (let anju of vien)
            {
				if (budy === anju)
                    ...
                    {
                    case 'Gm': case 'gm': case 'Good morning': case 'ගුඩ් මෝනින්':{
					result = fs.readFileSync(`./BixbyMedia/vn/gm.mp3`)
					GojoMdNx.sendMessage(m.chat, { audio: result, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
                    }break
                    case 'පලයන්': case 'පල': case 'Pala': case 'Palayn': case 'pala': case 'Pamka': case 'Pakaya': case "Hutta": case 'පම්ක': case 'පකයා':{
                        result = fs.readFileSync(`./BixbyMedia/vn/pala.mp3`)
                        GojoMdNx.sendMessage(m.chat, { audio: result, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
                        }
                            break
                    case "😎": {
					result = fs.readFileSync(`./BixbyMedia/vn/tara.mp3`)
					GojoMdNx.sendMessage(m.chat, { audio: result, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
                    }
                    break
                    case "😂": case "🤣": case "😅": {
					result = fs.readFileSync(`./BixbyMedia/vn/haha.mp3`)
					GojoMdNx.sendMessage(m.chat, { audio: result, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
                    }
                    break
                    case "😔": case "😢": case "😭": case "😥": case "😪":
                    {
					result = fs.readFileSync(`./BixbyMedia/vn/sad.mp3`)
					GojoMdNx.sendMessage(m.chat, { audio: result, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
                    }
                    break
...
\n│🔥💫.  Runtime : ${runtime(process.uptime())}\n\n╰──────────────────
 \n\n`,
							...
							},
							{
								...
								"rows": [
									...
									},
                                    {
                                        ...
                                        {
											"title": "Database Menu (Admin only)",
										"description": "Displays The List Of Database Features",
										"rowId": `${prefix}databasemenu`
										},
                                       
                                        {
                                            "title": "Group & Owner Menu (Admin Only)",
                                            "description": "Displays The List Of Main Features",
    ...
    case 'donasi': case 'donate': case 'sewabot': case 'sewa': {
                GojoMdNx.sendMessage(m.chat, { image: { url: 'https://i.ibb.co/xY3s8js/DRAGON.jpg' }, caption: `*Hi ${m.pushName}*\nDonation section is currently down🥲 , I know you are happy but me 🥲💔\n` }, { quoted: m })
            }
            break
            case 'sc': case 'script': {
                reply(`GitHub : https://github.com/Anonyshy/Queen\n Dont Forget To Give Like\n\nYouTube : ${myweb}\nDont Forget Subscribe`)
            }
            break
...
┃║⚜️  ${prefix}report [bug]
┃╚═════════════✪
┗━━❦${pushname} ❦━⭓`,/*<View style={styles.container}>
<AdMobBanner bannerSize="Banner" adUnitId="ca-app-pub-9652594430054961/4110175717" servePersonalizedAds={false}/>
</View>,*/
...
await GojoMdNx.send5ButImg(from, `` + '' + ' ', `
Thanks to Me ( P.H. Suresh Shyaman)
Pikachu (Lakmal) For helping me answered my every doubts regard this project
DGXeon , NexusMw, QueenBixy ( 90% Credits goes to them ,in this script)
And Again Me (ශ්‍යාමන්) ⚜️ Who Helped Assemble This Script !!!`,unicorn, [{"urlButton": {"displayText": "YOUTUBE","url": `${myweb}`}},{"urlButton": {"displayText": "GIT HUB","url": `${sc}`}},{"quickReplyButton": {"displayText": "DONATE","id": 'donate'}},{"quickReplyButton": {"displayText": "OWNER","id": 'owner'}}] )
break
            default:
                if (budy.startsWith('=>')) {
